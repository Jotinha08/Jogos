...
void setcolor(int color){ //função para as cores
   HANDLE hOut = GetStdHandle(STD_OUTPUT_HANDLE);
   SetConsoleTextAttribute(hOut, color);
      return;
}
void gotoxy(int w,int z){ //função para usar o gotoxy
...
void caixa(){ //função pra o limite da caixa do jogo
int x;
setcolor(2);
for(x=2;x<22;x++){
 gotoxy(2,x);
...
srand(time(NULL)); //cria uma comida inicial
pm=(rand()%60)+2;
rm=(rand()%12)+2;
gotoxy(pm,rm);
printf("%c",208);
 
while (jg!='s'){ //while que funciona o jogo

while (jg!='s' && !(jg=kbhit())){
for(x=t; x>0; x--){ //for da cobrinha 
cp[x]=cp[x-1];
cr[x]=cr[x-1];
}
...
if(pm==cp[0] && rm==cr[0]){ //if quando o jogador pegar uma comida
    //aumenta o tamanho
    t++;
    //aumenta a pontuação
    pontos++;
...
for (x=1; x<t; x++){
    ...
    if (cr[0]==3 || cr[0]==22 || cp[0]==3 || cp[0]==74)
        jg='s';
}
